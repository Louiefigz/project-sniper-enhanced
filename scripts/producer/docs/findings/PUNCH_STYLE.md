...
**SUPERSEDES** `PROJECT_SNIPER/docs/PUNCH_STYLE.md` (v1, fingerprint-level, one
...
`target.pace == "punch"` (resolver `plan_lint_motion._pacing_profile`):
...
> (`punch-shout-lockup.html` + tokens.css `--font-serif-display` ChunkFive/OFL
...
> (`pacing_punch.punch_zoom_max: 1.45`). The §11 music-bed blocker is closed
...
- **G17 · Punch-step ceiling.** `MOTION["punch_in"].zoom_max = 1.25` clamps the
  measured C5 step band (+12..45%, max x1.44 DaG; outlier x1.92 Daf). A punch
...
**Reproduction test** (scratch copy of `c0679-intro`, 36.67s punch short:
...
closest Punch moments, same instruments both sides:

| Axis | Ours | Punch | Delta |
|---|---|---|---|
...
**Verdict:** the executable profile (`pacing_punch` floors) and the plan
...
grammar at the structural level; it fails Punch at the TYPOGRAPHY layer (serif
