...
> **PROJECT_SNIPER note (read first):** The upstream GitHub registry and `hyperframes add` described below do **not** apply in this repo — the pipeline never installs registry blocks/components. The REAL "registry" here is the set of hand-authored HTML compositions at `templates/motion/compositions/<kind>.html` (~46 of them). A comp is "registered" simply by existing — it is resolved by `graphics_render.comp_path()` and catalogued by `template_contract.template_catalog()`. Face-hole "takeover" comps are additionally registered in `scripts/producer/graphics/pip_hole.py` (`HOLE_BY_KIND` — the four `nateherk-*` kinds). To see the current catalog, glob `templates/motion/compositions/`. Everything below is upstream HeyGen product docs, kept for reference only.
