...
# Which SNIPER_BRAIN_PROVIDER the app must see for a subscription provider.
brain_for_provider() {
  case "$1" in codex) printf 'codex' ;; claude) printf 'legacy' ;; *) return 1 ;; esac
}

# The editor brain is chosen once, with install/use-provider.command, and every
# part of Sniper reads that one choice: the app, the editor window, sign-in, the
# doctor and every background edit. A provider set anywhere else would make them
# disagree, so it is refused rather than obeyed.
check_provider_settings() {  # provider-from-sniper.env brain-from-sniper.env
  local want
  if [ "${SNIPER_PROVIDER:-}" != "$1" ] || [ "${SNIPER_BRAIN_PROVIDER:-}" != "$2" ]; then
    fail "runtime/sniper.local.env sets SNIPER_PROVIDER or SNIPER_BRAIN_PROVIDER. The editor brain
  is chosen for the whole app with install/use-provider.command codex|claude, so the app, the
  editor window and background edits always agree. Remove that line and use that command."
  fi
  want="$(brain_for_provider "$1")" || fail "runtime/sniper.env names no editor brain.
  Run install/use-provider.command codex  (or claude)."
  [ "$2" = "$want" ] || fail "runtime/sniper.env is inconsistent (provider $1, brain $2).
  Run install/use-provider.command $1 to rewrite it."
  model_settings_ok
}

# The same model settings reach the editor window (as CLI flags) and the app.
model_settings_ok() {
  case "${SNIPER_CODEX_REASONING:-}" in
    none|minimal|low|medium|high|xhigh|max|ultra) ;;
    *) fail "SNIPER_CODEX_REASONING must be one of none minimal low medium high xhigh max ultra." ;;
  esac
  local name value rest
  for name in SNIPER_CLAUDE_MODEL SNIPER_CODEX_MODEL; do
    value="${!name:-}"
    rest="$(printf '%s' "$value" | LC_ALL=C tr -d 'A-Za-z0-9._:/@+[]-')"
    case "$value" in ''|-*) rest="x" ;; esac
    [ -z "$rest" ] || fail "$name must be a model name such as opus or gpt-5.6-sol
  (letters, digits and . _ : / @ + [ ] -, not starting with -); found: '$value'"
  done
}
