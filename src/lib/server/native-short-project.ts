...
import { assertNativeShortPrebuildReview, assertNativePrebuildPublication, nativePrebuildReviewStatus, type NativePrebuildReviewBinding } from "./native-short-prebuild-review";
...
export function writeNativeShortProject(input: NativeShortProjectInput, destination: string, dependencies: NativeGuidedDependencies = {}) {
  ...
  const started = performance.now(), review = assertNativeShortPrebuildReview(input);
  ...
  const files = { ...projectFiles(input, html, catalog, media), "PREBUILD-REVIEW.json": canonicalJson(review) };
  ...
  assertGuidedNativeLocation(input, { directory }, dependencies);
  assertGuidedNativeBinding(input, dependencies);
  assertNativePrebuildPublication(input, directory, nativePrebuildReviewStatus(input, review));
  const manifest = { schemaVersion: 1, scope: "native-short-review-project", projectHash: canonicalJsonSha256(input),
    ...
    reviewMethod: input.strategy.review.method, prebuildReview: nativePrebuildReviewStatus(input, review),
