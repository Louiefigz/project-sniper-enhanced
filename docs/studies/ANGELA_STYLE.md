# ANGELA_STYLE.md — THE ANGELA STYLE grammar (4-reel granular synthesis)

**Creator:** angela (IG **@personalbrandlaunch**, social-media-marketing educator).
...
Study indexes: `~/ProjectSniper/_references/angela/<id>.study/fingerprint.json`.

**Confidence policy** (same standard as
`scripts/producer/docs/findings/JADEN_STYLE.md`): every rule carries its
...
Wired into the pipeline: `producer_config.MODES["short"]["pacing_angela"]`
(selected by `target.pace == "angela"`, generic resolver
...
misses Jaden's: Angela's camera cuts are big (takeover boundaries, all found)
...
Any pacing audit of an angela-style cut MUST count state events, not cuts.
...
black/white/charcoal plus micro-accents. This is the opposite of Jaden's
...
| Payload font | ExtraBold extended geometric grotesque, ALL CAPS (Archivo Black / Montserrat 900 / Poppins 800 class) — **NOT serif** (vs Jaden's Cooper) | 4/4 |
...
## 4. Camera grammar — the anti-Jaden
...
tight↔wide doctrine (JADEN_STYLE §2 C5): an angela plan runs `punchIns` EMPTY
...
@31.05. Jaden promotes to headroom; Angela shouts in the caption slot. Shared
...
Jaden E14: replicate the plain hand-set SPEED, never the typos.
...
Jaden M3. Start at gapDb≈10 and mix toward the measured floor by ear.
...
unchanged from the Jaden doctrine.
...
`producer_config.MODES["short"]["pacing_angela"]`, selected by
`target.pace == "angela"` (generic resolver `plan_lint_motion._pacing_profile`
...
4. **Density arithmetic:** her measured deck cadence (a slide/graphic event
   ...
   angela-treated intro passes the region-aware gate on graphics alone.
...
- **The zero-punch camera (AZ1).** Our longform envelope measures real
  ...
  pushes from a longform intro on Angela's authority — her corpus cannot
  speak to longform. Resolution: Angela supplies the GRAPHICS vocabulary
...
**#1 — `angela-takeover-deck` (lime slide-deck container + paging).**
...
**#2 — `angela-staircase-lockup` (kicker / lime payload / co-word + in-place
caption shout).** 4/4 reels, ~40 sightings — extends the Jaden G3 lockup with
...
**#3 — `angela-receipt-cell` (thumb cell + eye-count view chip; strips /
...
**#4 — `angela-caption-dual-mode` (whisper preset + canvas pill skin).**
...
**#5 — `angela-mock-os-cta` (fake-UI CTA bar).** 3/4 reels — high
...
**#6 — `angela-split-ledger` (persistent scoreboard + fly-dock absorb).**
