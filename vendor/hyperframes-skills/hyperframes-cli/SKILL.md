...
## PROJECT_SNIPER usage — the ONLY hyperframes call this repo makes

Everything above is upstream HeyGen product surface. In PROJECT_SNIPER the CLI is invoked in exactly one way: a pinned `render` subprocess spawned by `scripts/producer/graphics/graphics_render.py` (`_render_to`, lines ~152-154):

```bash
npx --yes hyperframes@0.7.33 render <motion_dir> -c <composition> --format {mov|mp4} --variables <json> -o <out>
```

- **`hyperframes@0.7.33` is pinned** (`graphics_render.HYPERFRAMES_PACKAGE`, line 54) — do not float the version; a template or token change already invalidates the content-hash render cache, so the binary must stay fixed too.
- **`-c` / `--composition` is load-bearing and undocumented upstream.** It selects which hand-authored comp under `templates/motion/compositions/` renders (`graphics_render` passes a throwaway `_gs-<hash>.html` copy of the target comp with the root `data-duration` rewritten to the entry's window). `<motion_dir>` is the render root; `-c` is the composition path relative to it.
- **`--variables <json>`** is the entry's spec, passed inline as `json.dumps(spec)` (line 154) — it fills the comp's `data-composition-variables`. It can NOT change render length (that's the root `data-duration` attribute, fixed at compile time).
- **`--format` follows the anchor, and overlays are alpha:** own-screen takeover graphics render **opaque `mp4`**; every overlay anchor renders **ProRes 4444 alpha `mov`** (`_FORMAT_BY_ANCHOR`, lines 61-69). **Never `webm`** — it drops the alpha channel (proven, edge G2; header comment lines 12-16). ProRes `.mov` is the only alpha-preserving product here.

The other subcommands sibling skills reference — `transcribe`, `present`, `play`, `auth`, `skills`, `capture`, `remove-background` (and `init`/`add`/`lint`/`validate`/`inspect`/`keyframes`/`lambda`/`publish`) — are upstream/workflow-specific and are **not** part of this repo's render path.
