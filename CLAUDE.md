...
- `SNIPER_BRAIN_PROVIDER=legacy|codex`; Codex defaults to `gpt-5.6-sol` + `ultra` and is configurable with `SNIPER_CODEX_MODEL` / `SNIPER_CODEX_REASONING`.
...
- **Provider/model pins.** The live/legacy `claude-sonnet-4-6` defaults remain intentional and must not be removed or silently changed. Local mode intentionally selects the Codex subscription bridge with `gpt-5.6-sol` + `ultra`; do not silently downgrade it. FRAME.IO REVIEW remains Anthropic vision and is outside the three-tool migration.
