# JADEN_STYLE.md — the iamjadenly shorts grammar (measured)

> **SUPERSEDED 2026-07-09** by
> `scripts/producer/docs/findings/JADEN_STYLE.md` — the 6-reel GRANULAR
...
| Level | Source |
|---|---|
| Fingerprints (ALL 6 reels) | `_references/iamjadenly/<id>.study/report.md` — scdet cuts, shot lengths, state timeline (fps=2.0), LUFS/crest/silence audio pass |
...
Wired into the pipeline: `producer_config.MODES["short"]["pacing_jadenly"]`
(selected by `target.pace == "jadenly"`, resolver in
...
`pacing_jadenly.min_changes_per_min = 10.0`.
...
`white-flash` / `light-leak` seam covers in a jadenly plan.
...
`MOTION["zoom"]["by_mode"]["short"]` (step_median 1.18); jadenly sits inside it.
...
(median ~19s). Encoded as `pacing_jadenly.max_still_gap_s = 20.0`. The hold is
...
This means a jadenly plan uses `captions_style: "minimal"`-class rendering
...
Encoded as `pacing_jadenly.music = True` (advisory; plan.music enables it).
...
(`AUDIO.music_gap_db` 10–12 dB rise-in-gaps / 18–20 dB duck): jadenly reads as a
...
`target.pace == "jadenly"` (resolver `plan_lint_motion._pacing_profile` maps
...
Plan-authoring checklist for a jadenly short (brain-side, from §§1–5):
...
- **G5 · overlay mind-map + triangle diagram (T3).** `whiteboard-map.html` is a
  FULL-FRAME OPAQUE graph-paper takeover (own-screen). Jaden's spokes/triangle
...
- **G10 · pop/whip animation tokens.** House motion language is fade/slide
  (power3.out) with self fade-outs. Jaden needs shared tokens: `pop-in` (≤83ms,
