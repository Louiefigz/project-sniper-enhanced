...
| 3 | Metadata lower-third (dark panel) | 7.0–13.0s | bottom-left panel: cyan mono eyebrow `PRODUCTION DISCLOSURE`; bold white 2-line `[NAME] DID NOT RECORD THIS`; divider; KV rows `ON SCREEN → [NAME] AVATAR V`, `WORKFLOW → AI GENERATED` (green) | headline + KV ledger | disclosure claim — facts as key→value receipts | eyebrow → headline → rows | 1 |
