...
> **PROJECT_SNIPER note (read first):** In this repo, HyperFrames is used **only** as the pinned `hyperframes@0.7.33 render` subprocess driven by `scripts/producer/graphics/graphics_render.py`. The creation workflows, `init`, the registry (`add`), the `lint`/`validate`/`inspect`/`keyframes` diagnostics, and the media/audio engine described below are **upstream HeyGen product docs and are NOT part of this pipeline.** For how graphics actually render here, read `scripts/producer/CLAUDE.md` (Graphics section) + `docs/producer/PRODUCER_MOTION_GRAPHICS_PLAN.md`. For the SNIPER↔HyperFrames decision (when to use the pipeline vs these skills), see the repo root `CLAUDE.md`.
