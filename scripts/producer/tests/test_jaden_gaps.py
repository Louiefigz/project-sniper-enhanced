"""Jaden reproduction-gap tests (G1-G5, G17 + music assets — JADEN_STYLE.md).
...
def _gfx(**over) -> dict:
    entry = {"outStart": 8.0, "outEnd": 12.0, "kind": "jaden-shout-lockup",
...
class PunchCeilingTests(unittest.TestCase):
    ...
    def test_jadenly_cap_admits_measured_band(self) -> None:
        # Measured biggest legal step x1.44 (DaG @17.0) -> profile cap 1.45.
        self.assertFalse(self._errors(1.35, pace="jadenly"))
        self.assertFalse(self._errors(1.45, pace="jadenly"))

    def test_jadenly_cap_rejects_outlier(self) -> None:
        # The x1.92 Daf outlier is NOT admitted.
        self.assertTrue(self._errors(1.5, pace="jadenly"))
    ...
    def test_profile_value_inside_primitive_hard_band(self) -> None:
        profile = plm.MODES["short"]["pacing_jadenly"]
...
class ShoutLockupTemplateTests(unittest.TestCase):
    COMP = _REPO / "templates" / "motion" / "compositions" / "jaden-shout-lockup.html"
    ...
    def test_comp_exists_with_house_conventions(self) -> None:
        ...
        self.assertIn('data-composition-id="jaden-shout-lockup"', html)
