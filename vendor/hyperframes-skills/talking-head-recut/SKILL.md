...
# confirm bundled vendor asset (gsap is REQUIRED):
ls "<SKILL_DIR>/assets/vendor/gsap.min.js"
# fonts are OPTIONAL and NOT shipped — a missing fonts dir is a WARNING, not a hard failure:
ls "<SKILL_DIR>/assets/fonts" 2>/dev/null \
  || echo "WARNING: no bundled fonts — compositions fall back to system fonts (see Step 9)"
```

Required:

- `ffmpeg` / `ffprobe` (system)
- `<SKILL_DIR>/assets/vendor/gsap.min.js` (bundled inside this skill, staged to work dir in Step 9)

Optional (NOT shipped):

- `<SKILL_DIR>/assets/fonts/*.woff2` — the branded fonts (`Caveat`, `Inter`, `Virgil`, `LXGW WenKai TC`) are **not bundled**. Drop your own licensed woff2 files into `assets/fonts/` if you want them; otherwise every `@font-face` below falls back to a system font stack and rendering still succeeds.
...
Font families the compositions reference: `Caveat` (handwriting),
`LXGW WenKai TC` (Chinese hand-script), `Inter` (modern sans), `Virgil`
(geometric hand). Their woff2 files are **NOT shipped** — drop your own into
`<SKILL_DIR>/assets/fonts/` (staged to the work dir in Step 9 and loaded via
`@font-face`); otherwise each `font-family` falls back to the system stack in the
`@font-face` block below. Reference via `@font-face` or `font-family` directly.
...
<div class="card" data-card-id="{cardId}">
  <style>
    ...
    .card[data-card-id="card-01"] .root {
      ...
      display: flex; ...;
      /* System fallbacks so a missing (unshipped) Caveat woff2 still renders. */
      font-family: 'Caveat', 'LXGW WenKai TC', ui-rounded, 'Comic Sans MS', cursive, serif;
      color: var(--text);
      background: var(--bg);
    }
...
# Fonts are OPTIONAL and NOT shipped — tolerate an empty/missing fonts dir. If no
# woff2 files are present, the @font-face rules below simply fall back to system fonts.
cp -n "$SKILL_DIR/assets/fonts/"*            "$WORK_DIR/public/fonts/" 2>/dev/null \
  || echo "note: no bundled fonts to stage — compositions use the @font-face system fallbacks"
...
<html lang="en">
  <head>
    <meta charset="utf-8" />
    <style>
      /* NOTE: these woff2 files are NOT shipped with the skill. If they are absent
         from public/fonts/, the browser skips the @font-face and each font-family
         stack below falls through to its system fallback (handwriting → ui-rounded /
         Comic Sans; sans → system-ui). Rendering still succeeds — only the exact
         branded typeface is lost. Provide your own licensed woff2 to restore it. */
      @font-face {
        font-family: "Caveat";
      ...
      :root {
        ...
        --accent-3: #e8590c;
        --accent-4: #9c36b5;
        /* System fallbacks after the (unshipped) branded fonts so rendering never breaks. */
        --font-family: "Caveat", "LXGW WenKai TC", ui-rounded, "Comic Sans MS", cursive, serif;
      }
      * {
