...
> **Shared render engine.** This skill shares its render engine (`build-frame.mjs` / `audio.mjs` / `captions.mjs` / `transitions.mjs` / `assemble-index.mjs`) with `product-launch-video`, `faceless-explainer`, and `pr-to-video`; they differ only in how Step 1 ingests source (URL capture / synthetic / `gh` PR). If you've read one, you mostly know the others.
