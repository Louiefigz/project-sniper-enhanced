...
const nplRenderer = {
  ...
  layout(context) {
    ...
    if (!["full-canvas", "caption-safe-upper-v1", "teaching-full-width-v1"].includes(vars.layout)) {
      throw new Error("nateherk-pipeline: unsupported explicit layout");
    }
    if (vars.layout === "caption-safe-upper-v1") {
      ...
      if (presenter || vars.exit !== "hold" || count < 2 || count > 6) {
        throw new Error("nateherk-pipeline: upper layout requires 2-6 nodes, hold exit and no presenter hole");
      }
    }
    if (vars.layout === "teaching-full-width-v1") {
      ...
      if (!noPresenter || count < 2 || count > 6 || !["hold", "blur-recede"].includes(vars.exit)) {
        throw new Error("nateherk-pipeline: teaching layout requires 2-6 nodes, a supported exit and no presenter hole");
      }
    }
  ...
  headline(vars) {
    ...
    if (lines.length > 2) {
      throw new Error("nateherk-pipeline: headlineLines is 2 lines max (§1.1 thesis grammar), got " + lines.length);
    }
  ...
  node(line, index, activeIndex) {
    ...
    if (fields.length !== 2 || !fields[0] || !fields[1]) {
      throw new Error("nateherk-pipeline: node " + (index + 1) + " needs num~label, got '" + line + "'");
    }
  ...
  chain(vars) {
    ...
    if (lines.length < 2 || lines.length > 8) {
      throw new Error("nateherk-pipeline: nodes needs 2-8 tiles, got " + lines.length);
    }
    ...
    if (!isFinite(activeIndex) || activeIndex < 0 || activeIndex > lines.length || activeIndex % 1 !== 0) {
      throw new Error("nateherk-pipeline: activeIndex must be an integer in [0," + lines.length +
  ...
  foot(vars) {
    ...
    if (accent === "process" || accent === "warn") element.classList.add(accent);
      ...
      throw new Error("nateherk-pipeline: footAccent '" + accent + "' not in result|process|warn");
    }
  },
  lands(context) {
    ...
    if (raw.length && (lands.length !== modules.length || lands.some((item) => !isFinite(item)))) {
      throw new Error("nateherk-pipeline: moduleLands needs " + modules.length +
  ...
  animate(context) {
    ...
    window.__timelines["nateherk-pipeline"] = timeline;
  },
  boot() {
