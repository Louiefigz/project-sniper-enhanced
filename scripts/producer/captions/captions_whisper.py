...
Specification: scripts/producer/docs/findings/PUNCH_STYLE.md §4 (CAP1-CAP3)
and §9.2; also the plain caption layer of docs/studies/RESTRAINED_STYLE.md §4
CAP2 (no emphasis words). The base "whisper" layer is a small white
sentence-case sans slot that hard-replaces 1-3 word verbatim cues, soft
shadow, NO box / NO stroke, centred horizontally in the y0.60-0.64 band. There
is no karaoke sweep (CAP1). Emphasis is INLINE: payload words
(``captions.emphasisWords``) render in the accent from the cue's FIRST frame
via an in-event colour override — never swept in, never a separate styled
event (CAP3). Keyword promotions leave the captions entirely and become
lockups (the ``punch-shout-lockup`` comp, PUNCH_STYLE.md §5.4 E1).
