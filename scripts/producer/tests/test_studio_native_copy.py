...
def original() -> str:
    ...
    entry = {"kind": "marker-highlight", "instanceId": "gfx-01-marker-highlight",
             "outStart": 1, "outEnd": 3.5}
    return _rebuild_instance(entry, {"spec": {
        "text": "Review the system", "emphasisWord": "system", "drawAt": 0.9}})
...
def native_text(source: str, text: str) -> str:
    ...
    changed, count = re.subn(r'(<p id="mh-text"[^>]*>).*?(</p>)',
...
class StudioNativeCopyTests(unittest.TestCase):
        ...
        for text in ("Review timing clearly", "Use &lt;safe&gt; &amp; clear copy", "Second edit"):
            with self.subTest(text=text):
                self.assertEqual(prove_composition_copy(
                    source, native_text(source, text), {}, "marker-highlight"),
                    {"text": html.unescape(text)})

    def test_native_undo_and_equivalent_serialization_have_no_change(self) -> None:
        source = original()
        self.assertEqual(prove_composition_copy(source, serialized(source), {}, "marker-highlight"), {})
        self.assertEqual(prove_composition_copy(source, source, {}, "marker-highlight"), {})

    def test_restoring_runtime_original_copy_is_not_a_new_semantic_value(self) -> None:
        source = original()
        native = prove_composition_copy(source, native_text(source, "Review the system"), {}, "marker-highlight")
    ...
    def test_code_geometry_structure_and_other_defaults_never_hitchhike(self) -> None:
        ...
        mutations = [
            copied.replace('id="mh-text"', 'id="mh-text" style="color: red"'),
            copied.replace('data-duration="60"', 'data-duration="90"'),
            copied.replace('const characters = Math.max(1,', 'const characters = Math.max(2,'),
            copied.replace('text: "The hook does the heavy lifting"', 'text: "The hook does more lifting"'),
            copied.replace('id="mh-text"', 'id="mh-text" id="other"'),
            copied.replace('</template>', '<p id="mh-text">Extra</p></template>'),
            copied.replace('&quot;default&quot;: 0.9,', '&quot;default&quot;: 1.9,'),
        ]
        for changed in mutations:
            with self.subTest(changed=changed != copied), self.assertRaises(ValueError):
                self.assertNotEqual(changed, copied)
                prove_composition_copy(source, changed, {}, "marker-highlight")
    ...
    def test_markup_comments_self_closing_and_duplicate_leaves_are_blocked(self) -> None:
        ...
        for text in ('<b>Changed</b>', '<script>alert(1)</script>', '<!--not copy-->Text', '<br>'):
            with self.subTest(text=text), self.assertRaises(ValueError):
                prove_composition_copy(source, native_text(source, text), {}, "marker-highlight")
        self_closed = re.sub(r'(<p id="mh-text"[^>]*>).*?</p>',
                             lambda match: match[1][:-1] + '/>', source)
        self.assertNotEqual(self_closed, source)
        with self.assertRaises(ValueError):
            prove_composition_copy(source, self_closed, {}, "marker-highlight")

    def test_host_and_native_disagreement_requires_resolution(self) -> None:
        source = original()
        with self.assertRaisesRegex(ValueError, "conflict"):
            prove_composition_copy(source, native_text(source, "Native"), {"text": "Host"}, "marker-highlight")
        self.assertEqual(prove_composition_copy(source, native_text(source, "Same"),
                         {"text": "Same"}, "marker-highlight"), {"text": "Same"})
    ...
    def test_unregistered_source_assignment_and_other_kinds_have_no_reverse_mapping(self) -> None:
        ...
        changed_source = source.replace('? vars.text :', '? vars.other :')
        with self.assertRaisesRegex(ValueError, "unproved"):
                prove_composition_copy(changed_source, native_text(changed_source, "Changed"), {}, "marker-highlight")
    ...
    def test_cold_preview_binding_preserves_native_copy_and_unedited_variables(self) -> None:
        ...
        cases = [(seed, "Host edit", "Host edit"), ("Native edit", seed, "Native edit"),
                 ("", seed, ""), (seed, seed, seed)]
        script = "const cases = " + json.dumps(cases) + "; const output = cases.map(([leafText, value]) => {"
        script += "const node = {textContent: leafText}; const document = {getElementById: () => node};"
        script += "const vars = {text: value}; " + assignment + " return [text, node.textContent];});"
        ...
        self.assertEqual(json.loads(result.stdout), [[" ".join(case[2].split()), ""] for case in cases])
        self.assertNotIn("</script>", assignment)

    def test_generated_leaf_contains_the_literal_seed_and_blank_native_edit(self) -> None:
        source = original()
        self.assertIn('>Review the system</p>', source)
        self.assertEqual(prove_composition_copy(source, native_text(source, ""), {}, "marker-highlight"), {"text": ""})

    def test_carriage_returns_are_entity_bound_and_nul_is_explicitly_unsupported(self) -> None:
        entry = {"kind": "marker-highlight", "instanceId": "gfx-01-marker-highlight",
                 "outStart": 1, "outEnd": 3.5}
        seed = "First\r\nSecond\rThird"
        source = _rebuild_instance(entry, {"spec": {"text": seed, "emphasisWord": "First"}})
        self.assertIn("First&#13;\nSecond&#13;Third</p>", source)
        self.assertEqual(prove_composition_copy(source, source, {}, "marker-highlight"), {})
        with self.assertRaisesRegex(StudioProjectError, "NUL"):
            _rebuild_instance(entry, {"spec": {"text": "No\x00guess", "emphasisWord": "guess"}})
        with self.assertRaisesRegex(ValueError, "NUL"):
            prove_composition_copy(source, native_text(source, "No\x00guess"), {}, "marker-highlight")
    ...
    def test_declaration_panel_default_only_matches_exact_supported_pair(self) -> None:
        ...
        self.assertEqual(prove_composition_copy(source, changed, {"text": "Changed"}, "marker-highlight"), {})
        with self.assertRaises(ValueError):
            prove_composition_copy(source, changed, {}, "marker-highlight")
