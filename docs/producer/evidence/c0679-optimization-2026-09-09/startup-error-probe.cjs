...
function attachStartupDiagnostics() {
  window.__nativeStartupIssues=[];
  function safeUrl(value) {
    try {const url=new URL(value,location.href);return url.origin+url.pathname;}catch{return null;}
  }
  function record(event) {
    if(window.__nativeStartupIssues.length>=100)return;
    const reason=event.reason||event.error||event;
    const target=reason.target||event.target;
    window.__nativeStartupIssues.push({type:event.type,wall:performance.now(),
      reasonType:reason?.constructor?.name,message:reason?.message||String(reason),
      reasonEvent:reason?.type,targetType:target?.constructor?.name,targetTag:target?.tagName,
      targetUrl:safeUrl(target?.url||target?.src||target?.href||event.filename||''),
      file:safeUrl(event.filename||''),line:event.lineno,stack:reason?.stack});
  }
  window.addEventListener('unhandledrejection',record);
  window.addEventListener('error',record,true);
}

async function startupIssues(page) {
  const results=[];
  for(const frame of page.frames()) {
    results.push({url:frame.url().split('?')[0],issues:await frame.evaluate(()=>window.__nativeStartupIssues||[])
...
async function evaluatePage(page, frame, config, result) {
  if(config.diagnosticsOnly) {
    await new Promise(resolve=>setTimeout(resolve,2000));
    result.startupIssues=await startupIssues(page);
    result.status='diagnostic-observation-complete';
    return;
  }
  ...
  result.audioEnabledThroughout=result.playback.samples.filter(sample=>sample.clock>.5)
    .every(sample=>!sample.audioMuted&&sample.audioVolume>0&&!sample.audioPaused);
  result.startupIssues=await startupIssues(page);
  result.status=result.opening.passed&&result.complaintWindow.passed&&result.audioEvents.passed
    &&result.audioEnabledThroughout&&!result.playback.overflow&&!result.errors.length
    &&!result.badResponses.length?'telemetry-passed':'telemetry-failed';
}
...
async function run(config) {
  ...
  try {
    ...
    await page.evaluateOnNewDocument(attachStartupDiagnostics);
    page.on('pageerror',error=>{if(result.errors.length<100)result.errors.push({message:String(error),name:error.name,stack:error.stack,wall:Date.now()-started});});
    ...
    await evaluatePage(page,frame,config,result);
  ...
  return ['telemetry-passed','diagnostic-observation-complete'].includes(result.status)?0:1;
}
