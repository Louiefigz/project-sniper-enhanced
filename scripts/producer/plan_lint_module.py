#!/usr/bin/env python3
"""plan_lint_module — lint for the MODULE longform lane (§5 items 8-9).
...
* an ACTIVE hole-comp (``graphics.pip_hole.entry_has_hole`` — always for
  ``module-takeover``, opt-in via ``spec.presenterFrame`` for the dark
...
def _check_entrance(tag: str, g: dict, mode: str, rep: Any) -> None:
    ...
    if entrance == "rail-push" and mode != "longform":
        rep.error(f"{tag}: entrance 'rail-push' (rail beside the live face, "
                  "MODULE T-B) is LONGFORM-ONLY — banned for shorts "
...
def _check_hole_kind(tag: str, g: dict, mode: str, rep: Any) -> None:
    ...
    active presenter-frame (or module-takeover) draws the longform+own-screen
    gate."""
    if not entry_has_hole(g):
        return
    if mode != "longform":
        rep.error(f"{tag}: {g.get('kind')} (face-in-PIP takeover, MODULE "
...
def check_module_entry(tag: str, g: dict, mode: str, rep: Any) -> None:
    """All MODULE-lane checks for one graphics entry."""
