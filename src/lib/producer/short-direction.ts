import { visualStorytellingInstructions, type VisualStorytellingStage } from "./visual-storytelling";
...
/** Apply shared source scouting before choosing the beat-specific explanation. */
const SHORT_VISUAL_STORYTELLING_INSTRUCTIONS = `For each beat, establish the teaching purpose, then apply the shared real-asset scouting decision before committing to its visual representation. In a produced treatment, source-first describes search order; it does not mean local-only. Reuse the authorized media policy exactly and inspect real candidates before deciding that an illustration is the better explanation. A conceptual overview may be clearest through a familiar physical object, cartoon metaphor or simple diagram; a procedure needs the relevant real interface and visible actions; a factual claim needs appropriate observed evidence. Presenter performance can carry a beat when an added visual would not help. Make this choice from the complete spoken point, audience understanding and story, not a keyword-to-asset rule or a whole-Short preset.
...
export function shortDirectionInstructions(value?: ShortDirectionRequest, stage: VisualStorytellingStage = "author"): string {
