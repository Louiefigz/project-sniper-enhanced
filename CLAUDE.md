...
Run `/sniper-context` (Codex: `$sniper-context`) to locate installed tools,
HyperFrames skills, the applicable instruction chain and an explicitly selected
project's current handoff. CLI: `python3 -B scripts/producer/context.py
--project /absolute/project`. Read the applicable files it locates; the report
does not complete instruction reading, setup or review.
...
`clipper`, `segmenter`, `reference-editor`, `producer-study`, and `sniper-context`.
...
**SNIPER ↔ HyperFrames routes.** Current native Short/Long projects follow
`docs/PIPELINE.md`, the installed `hyperframes` entry/domain skills, the declared
project contract and the shared `scripts/producer/studio/native_export.py`
supervisor. Studio is the editable review surface. Existing `edit_plan.json`
Producer jobs retain their applicable deterministic compatibility pipeline.
The native route supersedes the old graphics-only boundary below or in archived
vendor documentation. `vendor/hyperframes-skills/` is historical provenance;
installed domain skills are reported by `sniper-context` and read before use.
`vendor/hyperframes-catalog/` supplies local source references; its historical
lock count is separate from current readable files. Catalog presence does not
qualify a composition or bypass the route's reviews and export gates.
...
a fresh independent critic verdict. For existing Producer plans, the in-house
`assemble.py`/ffmpeg chain remains the deterministic renderer; native Short/Long
projects use the declared native route above and must not be rerouted into
...
- `SNIPER_TRANSCRIBE_PROVIDER` defaults to `local-whisper` in every execution mode. Environment variables never authorize paid ASR. Deepgram requires explicit approval for the current invocation and the paired `--provider deepgram --authorize-paid-asr deepgram` arguments, enforced by `scripts/asr_policy.py`. Never fall back to a paid provider automatically.
...
- In the buyer package, connect, replace or remove this key with `../install/setup.command`. The launcher loads the private `../runtime/deepgram.env` file. Do not ask the operator to paste secrets into chat, display the file, or treat a saved key as spending consent. Deepgram uploads audio and bills separately when explicitly used.
