# NATEHERK STUDY — J_jswzXhYJA ("How this video rendered itself")

Date: 2026-07-10. Source video: 323s, 1920x1080@30 (`_references/nateherk/J_jswzXhYJA.webm`,
...
`_references/nateherk/J_jswzXhYJA.study/` (working copies in the session scratchpad
`nateherk-study/{sheets,bursts}/`). Timing uncertainty ±1 frame (~40ms). N=1 video — these are
priors, not laws.
...
| 4 | Mining the creator's prior work for style | **OURS BETTER** | His is ad-hoc "took some inspiration". Ours is producer-study: yt-dlp → fingerprint → frame-by-frame study → grammar doc with receipts → executable pacing profile (Caleb 9 cuts/146s; Jaden punch band 1.30–1.45, 50/53 rules frame-verified; Angela comps). |
...
tighter TH crop.
**Verdict: ALREADY OURS.** `angela-takeover-deck` arrives 0-frame on a hard cut;
...
- **No change to `white-flash`/`light-leak`.** His video has no flash/leak grammar; those
  ...
  nateherk grammar is a new TREATMENT lane, not a replacement.
...
`POP_IN_S` (≤2 frames), `POP_SCALE_FROM` (0.94). Jaden pack law (pop ≤2f, never alpha fade)
...
| 6 | **Semantic dual accent** — lime = hero/results, cyan = process/status, gray = comparison | Benchmark hero bar lime vs comparison gray; cyan eyebrows on process cards; consistent across all 14 takeovers | MED-HIGH — system-level cohesion | **S–M** | `tokens.css`: add `--accent-result` / `--accent-process` / `--accent-compare` (theme maps them; nateherk skin: `#c6f542` / `#35b6d9` / `#9aa0a6`). `planner/graphics_planner_style.py` assigns the semantic class per lane: gauge/receipts/payoff → result; boundaries/sequences/status → process. Deterministic mapping, no LLM. |
...
| 11 | **Light/dark two-mode alternation** — cream rail chapters vs dark PIP-canvas chapters as the video's rhythm | ~14 takeovers alternating LIGHT/DARK, one per narration beat, 9–18s holds | MED (longform 16:9 lane) | **M** | Light skin = token theme on `glass-rail` (T-B); dark canvas = new `nateherk-takeover.html` comp (near-black + grid texture + content column; PIP slot only if §4 approves — until then full-frame with no face hole, entered via hard cut per current doctrine). Hold ceilings already allow 9–18s? Check: `MOTION["hold_max_s"]["longform"] = 11.0`, `takeover_max_s = 10.5` — the reference holds 9–18s. If we adopt the lane, raise ONLY under a nateherk preset (mode-keyed override in `producer_config.py`), justified by the measured 9–18s band; do not flatten globally. |
...
`jaden-shout-lockup` (different lane, different law); our SAFE_BOX/free-space placement
...
the cutaway grammar is cut-carried (high cut density), the nateherk grammar is
...
1. **[S] Comp grammar micro-pack: eyebrow-first + two-line accent headline + opacity-first
   ...
   word-rise with in-place opacity ramp (`TEXT_RAMP_S = 0.2`) as the nateherk variant.
   Tokens in `templates/motion/tokens.css` + `motion-tokens.js`. Jaden pack untouched.
...
9. **[M] Dark-takeover comp + two-mode chapter rhythm (nateherk preset).**
   `nateherk-takeover.html` (near-black #0c1014 + grid, content column, semantic dual-accent
