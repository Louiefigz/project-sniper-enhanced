...
| X8 | MP4 container/stream duration inflated by AAC encoder padding | Picture duration is derived from counted video packets ÷ exact rational CFR; music fitting and timeline assertions never use container/stream duration. Mismatched `avg_frame_rate`/`r_frame_rate` evidence fails closed | **handled** (cut_speed 2026-07-04; music authority hardened 2026-07-29) |
| X9 | Final master can duplicate tail frames when CFR fills an AAC-padded mezzanine tail | Master receives the pre-master picture frame count and passes `-frames:v`; final video cannot grow to follow audio/timestamp tails | **handled** (2026-07-29) |
...
| X19 | Per-part AAC padding and concat timestamps can inflate the master; one real 4-part 23.976 fixture also exposed bogus concat `r_frame_rate=120/1`, turning a 19-frame mezzanine into roughly 96–98 final frames | Master rate comes from the first cut source's exact rational profile, duration comes from pre-master frame count ÷ that rate, and `-frames:v` caps the encode. The 4-part AAC regression preserves 19 frames at `24000/1001` with ≤50 ms A/V tail skew | **handled** (initial clamp 2026-07-05; authority fix 2026-07-29) |
| X20 | Master rounded fractional NTSC fps to integer (`-r 24` on a 23.976 source) or trusted bogus concat rate metadata → duplicated frames and frame-indexed seams sliding off target | `MasterSpec.fps_exact` carries the first cut source's rational rate verbatim to `-r`; integer fps remains only for GOP/bitrate lookup, and final frames are explicitly capped | **handled** (2026-07-06; source-rate authority regression 2026-07-29) |
| X21 | ORDERING LAW: any stage that MIXES audio (transitions' whooshes land on both channels) must run AFTER dead-channel repair — the mixed SFX makes a dead source channel look alive to master's detector, silently shipping one-eared speech (v2 intro first render: speech ch2-only at −68 dB in ch1 windows) | `render.channels_stage` (dual-mono via `master.dead_channel_prefix`) runs immediately after cut and before enhance → transitions → gain → master; an orchestration regression pins the complete ordering | **handled** (2026-07-06; ordering regression 2026-07-29) |
