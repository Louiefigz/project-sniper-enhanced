...
(evidence: scratchpad `caleb_trace/`, studies at
`_references/caleb/<id>.study/`). **Fresh-pixel spot-check 2026-07-09:** six
...
beside `scripts/producer/docs/findings/JADEN_STYLE.md` (locked-tripod
punch-cut shorts) and `docs/studies/MEASURED_EDIT_GRAMMAR.md` (pro long-form). What
JADEN does with cuts + a shout layer, CALEB does with captions alone.
...
jadenly's 51–89 states/min band — and every cue is **word-locked to speech**.
The knob that changes per grammar is WHICH layer carries the churn: pro
long-form = graphics, jadenly = cuts + lockups, **caleb = captions only**.
...
opposite of jadenly H2 (graphics-carried front-load) and the longform 2.35x
envelope — encoded as `pacing_caleb.hook_front_load = 1.0`.
...
`pacing_caleb.min_changes_per_min = 0.0`.
...
cut instant, which conceals the seam. This is the OPPOSITE of jadenly C4
...
stops (§4) — encoded as `pacing_caleb.max_still_gap_s = 40.0` (above the
...
ride clause-start cues (C3). Executable: when a caleb plan cuts at all, snap
...
@4.9s). Unlike jadenly there is NO shout layer to promote into — emphasis is
...
jadenly. Do NOT replicate the typo; DO replicate verbatim-over-groomed.
...
`pacing_caleb.music = False`.

**M2 — Master numbers. [3/3 — HIGH]**
Integrated −14.1…−14.6 LUFS — same family as the jadenly 6/6 pin and
...
EXCEEDING the −1.5 delivery ceiling — same anti-rule as jadenly M2, keep our
TP ceiling.
...
grammar the way it is for jadenly (255–260wpm stripped) — tighten only true
flubs (C2).
...
## 7. RESTRAINT — what caleb does NOT do (vs JADEN_STYLE)
...
| Device | JADEN (6-reel measured) | CALEB (3-reel measured) |
|---|---|---|
...
cutaways. Restraint is not a cheaper jadenly — it is a different instrument.
...
Per-event rules from single reels follow the JADEN_STYLE policy: hypotheses
...
`producer_config.MODES["short"]["pacing_caleb"]`, selected by
`target.pace == "caleb"` (resolver `plan_lint_motion._pacing_profile` maps
...
**CALEB calibrates `light`; JADEN stays the calibration for `produced`.**

The `edit_scope` ladder's `light` scope (base cut + captions + subtle motion,
graphics/transitions OFF) is, to within one comp, exactly the measured caleb
kit — and caleb is the only measured grammar that is ON-STYLE with the
engaging lanes dark. Concretely, a light short should default to:

- `target.pace = "caleb"` → `pacing_caleb` floors (a 0-cut 40s hold is
...
- Captions: the caleb whisper-verbatim variant (§4), NOT karaoke — karaoke is
  off-style in BOTH measured shorts grammars (jadenly CAP1, caleb CAP2); the
  Light preset is where that correction bites first.
- Motion: `punchIns` EMPTY — caleb measures ZERO creep, so `light`'s "subtle
...
- The ONE graphic (H1) is the deliberate exception to "graphics OFF": a
  static thesis title chip is part of the caleb base kit. Until CG1 ships,
  a `light` plan approximates it with a hook card — but note the hold-time
  gap below.

Jadenly, by contrast, is a heavy produced grammar (17.2 cuts/min, ~30
...
- **CG2 · cold-open thesis-card variant.** Dark card ≈0.76W x 0.18H, LOWER-MID
  ...
  spec only.)
- **CG3 · whisper-caption top-pinned variant.** Extends JADEN_STYLE gap G2
...
churn is handled by the `pacing_caleb` floors; the music-vote false-positive
