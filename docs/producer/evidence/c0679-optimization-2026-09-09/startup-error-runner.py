...
def settings(label: str, diagnostics_only: bool = False) -> dict:
    ...
    config.write_text(json.dumps({'url':'http://127.0.0.1:41058/#project/native-presenter-v1',
        ...
        'output':str(output),'diagnosticsOnly':diagnostics_only}, indent=2)+'\n')
    ...
    admission = {'output':str(output),'sdkSha256':digest(CLI),'sandboxSha256':digest(SANDBOX),
                 'sourceProgramDuration':683.7664166666667,'observationStart':0,'observationEnd':0 if diagnostics_only else 65,
                 'browserSha256':digest(CHROME),'jobKind':'studio-playback-observation'}
    return {'root':directory,'project':PROJECT,'cli':CLI,'deadline':50 if diagnostics_only else 135,
...
def execute(label: str, diagnostics_only: bool = False) -> dict:
    """Keep raw lifecycle receipts separate from observed playback quality."""
    values = settings(label, diagnostics_only)
...
def main() -> None:
    ...
    if not args.execute:
        ...
        return
    print(json.dumps(execute(args.label,args.diagnostics_only)))
