...
- **Words:** `--words <path>` writes the flat `[{id,text,start,end}]` shape below, directly compatible with the captions pipeline. HeyGen's `<start>`/`<end>` boundary sentinels are filtered out and ids are re-contiguous.
...
| Cloud TTS with a large voice catalog                      | **ElevenLabs**                                      |
...
When `--words <path>` is passed to a HeyGen call, the file is written in the same flat shape `transcribe` produces — directly compatible with the captions pipeline:
